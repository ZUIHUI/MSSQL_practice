...
begin
	if(@temp = 1)
		begin 
			insert into @table1
			select 姓名,交易次數 ,ROW_NUMBER() OVER(ORDER BY 交易次數 asc) from @table as RM
		end
	else if(@temp = 2)
		begin 
			insert into @table1
			select 姓名,交易次數 ,ROW_NUMBER() OVER(ORDER BY 交易次數 desc) from @table as RM
		end
	return
end
go
